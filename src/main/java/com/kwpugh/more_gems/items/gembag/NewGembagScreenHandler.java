...
public class NewGembagScreenHandler extends ScreenHandler
{
    ...
    private NewGembagScreenHandler(ScreenHandlerType<?> type, int syncId, PlayerInventory playerInventory, int rows) {
        this(type, syncId, playerInventory, new SimpleInventory(9 * rows), rows);
    }

    public static NewGembagScreenHandler createGeneric9x6(int syncId, PlayerInventory playerInventory) {
        return new NewGembagScreenHandler(ScreenHandlerType.GENERIC_9X6, syncId, playerInventory, 6);
    }

    public static NewGembagScreenHandler createGeneric9x6(int syncId, PlayerInventory playerInventory, Inventory inventory) {
        return new NewGembagScreenHandler(ScreenHandlerType.GENERIC_9X6, syncId, playerInventory, inventory, 6);
    }

    public NewGembagScreenHandler(ScreenHandlerType<?> type, int syncId, PlayerInventory playerInventory, Inventory inventory, int rows) {
        super(type, syncId);
        ...
        for(j = 0; j < this.rows; ++j) {
            for(k = 0; k < 9; ++k) {
                this.addSlot(new GembagSlot(inventory, k + j * 9, 8 + k * 18, 18 + j * 18));
            }
        }

        for(j = 0; j < 3; ++j) {
            for(k = 0; k < 9; ++k) {
                this.addSlot(new GembagSlot(playerInventory, k + j * 9 + 9, 8 + k * 18, 103 + j * 18 + i));
            }
        }

        for(j = 0; j < 9; ++j) {
            this.addSlot(new GembagSlot(playerInventory, j, 8 + j * 18, 161 + i));
        }

    }

    public static class GembagSlot extends Slot
    {
        public GembagSlot(Inventory inventory, int index, int x, int y)
        {
    ...
    public ItemStack quickMove(PlayerEntity player, int index) {
        ...
        if(testItem instanceof GembagItem ||
